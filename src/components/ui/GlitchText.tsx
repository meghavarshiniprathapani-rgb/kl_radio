...
const GlitchText: FC<GlitchTextProps> = ({
  ...
  const inlineStyles: CustomCSSProperties = {
    ...
    '--after-shadow': enableShadows ? '-5px 0 hsl(var(--primary))' : 'none',
    '--before-shadow': enableShadows ? '5px 0 cyan' : 'none'
  };

  const baseClasses = 'text-foreground text-[clamp(2rem,10vw,8rem)] font-black relative mx-auto select-none cursor-pointer';
